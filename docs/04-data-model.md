...
```ts
{
  ...
  profileTags: string[],          // up to 10 interest tags; used for Explore people directory
...
```ts
{
  ...
  // Context — only the relevant pointer(s) are set
  postId: ObjectId | null,
  commentId: ObjectId | null,
  messageId: ObjectId | null,
  conversationId: ObjectId | null,   // set for type='message'
...
**Indexes**
- `{ recipientId: 1, updatedAt: -1 }` — inbox view (paginated)
- `{ recipientId: 1, read: 1 }` — unread count
- `{ recipientId: 1, read: 1, updatedAt: -1 }` — composite: covers filter + sort in one scan (ESR rule)
...
**Indexes**
- `{ userId: 1, postId: 1 }` unique — one bookmark per user per post
- `{ userId: 1, createdAt: -1 }` — user's bookmark feed

### `apikeys`

Stores hashed API Keys for agent / programmatic access. Plaintext is never persisted.

```ts
{
  _id: ObjectId,
  userId: ObjectId,           // ref: users
  name: string,               // human label, ≤ 100 chars
  keyHash: string,            // SHA-256 hex of the raw key (unique)
...
**Indexes**
- `{ userId: 1 }` — list a user's keys
- `{ keyHash: 1 }` unique — lookup on every authenticated API request

### `events`

Lightweight analytics event log. Schemaless `context` field for flexibility. 90-day TTL keeps the collection from growing unboundedly.

```ts
{
  _id: ObjectId,
  type: string,               // e.g. 'post:view', 'profile:view'
